...
import { Plus, Search, Filter, Edit, Trash2, Eye, Calendar, MapPin } from 'lucide-react'
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-8">
          <div className="bg-white rounded-3xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <h3 className="text-2xl mb-6">Tambah Pertunjukan Baru</h3>
            <form className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-gray-700 mb-2">Judul Pertunjukan</label>
                  <input
                    type="text"
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Deskripsi pertunjukan"
                />
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm text-gray-700 mb-2">Harga (Rp)</label>
                  <input
                    type="number"
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500"
                    placeholder="75000"
                ...
                <div>
                  <label className="block text-sm text-gray-700 mb-2">Kapasitas</label>
                  <input
                    type="number"
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500"
                    placeholder="500"
                ...
                <div>
                  <label className="block text-sm text-gray-700 mb-2">Penyelenggara</label>
                  <input
                    type="text"
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500"
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-white rounded-3xl p-8 max-w-md mx-4 text-center">
            ...
            <h3 className="text-2xl mb-2">Hapus Pertunjukan?</h3>
            <p className="text-gray-600 mb-6">
              Apakah Anda yakin ingin menghapus pertunjukan ini? Tindakan ini tidak dapat dibatalkan.
            </p>
                  ...
                  <tr key={ticket.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <img src={ticket.imageUrl} alt={ticket.title} className="w-16 h-16 rounded-xl object-cover" />
                        <div>
                          <div className="text-gray-900 max-w-xs truncate">{ticket.title}</div>
                          <div className="text-sm text-gray-500">{ticket.organizer}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="px-3 py-1 bg-purple-100 text-purple-700 rounded-full text-sm">{ticket.category}</span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 text-gray-700">
                        <Calendar className="w-4 h-4 text-purple-500" />
                        <div>
                          <div className="text-sm">{new Date(ticket.date).toLocaleDateString('id-ID')}</div>
                    ...
                    <td className="px-6 py-4 text-gray-900">Rp {ticket.price.toLocaleString('id-ID')}</td>
                    <td className="px-6 py-4">
                      <div className="text-gray-900">
                        {ticket.ticketsSold} / {ticket.capacity}
                      </div>
                      <div className="text-xs text-gray-500">
                        {Math.round((ticket.ticketsSold / ticket.capacity) * 100)}% terjual
                      </div>
                    </td>
                    <td className="px-6 py-4">
